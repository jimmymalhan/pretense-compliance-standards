# DORA — synthetic compliance corpus
...
#   pretense scan frameworks/DORA/
# … or drive the synthetic corpus through the bridge:
node --experimental-transform-types \
  pretense_compliance_standards/pretense_bridge/run.mjs --framework DORA
```

## Run this framework's tests

```bash
uv run pytest tests/test_pcs.py -m dora -q --noconftest
```
