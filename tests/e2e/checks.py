...
def read_json(path: pathlib.Path, what: str, code: str) -> dict:
    ...
    if not raw.strip():
        fail(
            code,
            f"{what} at {path} is EMPTY (0 meaningful bytes). "
    ...
    try:
        ...
        fail(
            code,
            f"{what} at {path} is not valid UTF-8 "
    ...
    try:
        ...
        fail(
            code,
            f"{what} at {path} is not valid JSON: {exc.msg} "
...
def cmd_frameworks() -> None:
    doc = read_json(
        PKG / "compliance_map.json", "compliance_map.json", "E_MAP_UNREADABLE"
    )
...
def cmd_disk_counts(out_path: str) -> None:
    ...
    neg = read_json(
        PKG / "corpus" / "negatives.json",
        "benign look-alike corpus",
    ...
    for name in EXPECTED_FRAMEWORKS:
        ...
        try:
            ...
            bad.append(
                f"{name}: case file is NOT VALID UTF-8 (byte {exc.start}) — corrupt"
            )
            continue
        except json.JSONDecodeError as exc:
            bad.append(
                f"{name}: case file is NOT VALID JSON ({exc.msg} line {exc.lineno})"
            )
            continue
        ...
        if len(cases) == 0:
            bad.append(
                f"{name}: ZERO cases on disk — this is a FALSE GREEN, not a pass"
            )
            continue
    ...
    with open(out_path, "w", encoding="utf-8") as fh:
        for name, n in rows:
            fh.write(f"{name}\t{n}\n")
    print(
        f"[e2e] non-zero case count asserted for all {len(rows)} frameworks "
...
# --------------------------------------------------------------------------
def cmd_bridge_row(
    name: str, json_path: str, disk_count: str, baseline_path: str, row_out: str
) -> None:
    doc = read_json(
        pathlib.Path(json_path), f"bridge JSON for {name}", "E_BRIDGE_OUTPUT"
    )
    ...
    with open(row_out, "a", encoding="utf-8") as fh:
        fh.write(
            f"{name}\t{n}\t{egress}\t{rate:.1f}\t{identify}\t{irate:.1f}\t"
    ...
    print(
        f"[e2e]   {name:<14} cases={n:<4} egress={egress}/{n} = {rate:5.1f}%  "
...
def cmd_cross(hipaa_json: str, pci_json: str) -> None:
    ...
    for other in ("HIPAA", "HITECH"):
        if pf.get(other, {}).get("n", 0) != 0:
            problems.append(
                f"PCI_DSS-scoped run contains {pf[other]['n']} {other} cases"
            )
    ...
    print(
        f"[e2e] cross-contamination: 0 (probe live: HIPAA n={hf['HIPAA']['n']} over "
...
def cmd_summary(rows_path: str, pytest_path: str) -> None:
    ...
    print("\n" + "=" * w)
    print(
        "36-FRAMEWORK RESULT TABLE  (egress redaction = headline; identify = secondary)"
    )
    print("=" * w)
    print(
        f"{'#':>2}  {'FRAMEWORK':<14} {'CASES':>6} {'EGRESS':>8} {'RATE':>7} "
    ...
    for i, r in enumerate(rows, 1):
        ...
        if status != "PASS" or pstat != "5 passed":
            verdict_ok = False
        note = (
            "  <- FITTED TO FIXTURES, not evidence"
            if name in FITTED_TO_FIXTURES
            else ""
        )
        print(
            f"{i:>2}  {name:<14} {n:>6} {eg:>8} {rate:>6}% {irate:>6}% "
            f"{wk:>10} {miss:>5} {base:>6}% {pstat:>10} {status:>7}{note}"
        )
    print("-" * w)
    print(
        f"    frameworks={len(rows)}  distinct cases={EXPECTED_TOTAL_CASES}  "
    ...
    print(
        f"    INDEPENDENCE: {len(sets)} distinct kind-set configurations behind "
    ...
    print(
        "    Report the case count and the redaction rate. 36 is a LABEL count, "
    ...
    print(
        "VERDICT: PASS — all 36 frameworks measured a NON-ZERO case set, met their "
